...
import {
  ...
  Tag, Sparkles, Bike, CheckCircle2, Trash2, ChevronRight, Phone,
  UserPlus, Star, Calculator,
} from 'lucide-react';
import {
...
/** Resolve the display price for a menu item given the selected platform. */
function getDisplayPrice(item: MenuItem, platformId: number | null): number {
  if (!platformId || !item.deliveryPrices?.length) return item.price;
  const match = item.deliveryPrices.find(dp => dp.deliveryPlatformId === platformId);
  return match?.price ?? item.price;
}

/** Re-resolve price from a cart item's stored deliveryPrices when platform changes. */
function resolveCartPrice(basePrice: number, deliveryPrices: DeliveryPrice[], platformId: number | null): number {
  if (!platformId || !deliveryPrices?.length) return basePrice;
  const match = deliveryPrices.find(dp => dp.deliveryPlatformId === platformId);
        ...
        <div className="px-4 pb-4 space-y-3 border-t border-slate-50">
          ...
          {member && (
            <div className="flex items-center gap-3 p-3 bg-emerald-50 rounded-xl border border-emerald-100 mt-3">
              ...
              <button onClick={onRemoveMember} className="text-slate-300 hover:text-slate-500 shrink-0">
                <X className="w-4 h-4" />
              </button>
            </div>
          )}
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-dashed border-slate-200">
              ...
              {!showQuickReg ? (
                ...
                <div className="mt-2 space-y-2">
                  ...
                  />
                  <Button
                    size="sm" onClick={onRegister} disabled={isRegistering}
              ...
              {selectedPromo ? (
                <div className="flex items-center justify-between p-3 bg-primary/5 rounded-xl border border-primary/15">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => onApplyPromo(selectedPromo)} className="text-slate-300 hover:text-slate-500">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                </div>
...
export default function DeliveryService() {
  ...
  // ── Delivery Platforms (fetched from API) ──
  ...
  const [selectedOptions, setSelectedOptions] = useState<{ id: number; name: string; price: number }[]>([]);
  ...
  const totalAmount = cart.reduce((sum, item) => {
    return sum + (item.price + item.options.reduce((s, o) => s + o.price, 0)) * item.quantity;
  }, 0);
  const finalTotal = Math.max(0, totalAmount - discountAmount);

  // ── Derived: selected platform object ──
  const selectedPlatform = platforms.find(p => p.id === selectedPlatformId) ?? null;

  // ── Load menu + delivery platforms + promos ──
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [menuRes, platformsRes] = await Promise.all([
          api.get(`/branches/${branchId}/menu`),
          api.get(`/menus/delivery-platforms?branchId=${branchId}`),
        ]);
        ...
        const platformList: DeliveryPlatform[] = Array.isArray(platformsRes.data) ? platformsRes.data : [];
  ...
  // ── Re-price cart when platform changes ──
  useEffect(() => {
    if (cart.length === 0) return;
    setCart(prev => prev.map(item => ({
      ...item,
      price: resolveCartPrice(item.basePrice, item.deliveryPrices, selectedPlatformId),
    })));
  }, [selectedPlatformId]);

  // ── Clear member/promo ──
  const clearMemberAndPromo = () => {
    setMember(null); setMemberPhone(''); setMemberNotFound(false);
  ...
  const toggleOption = (group: MenuItem['optionGroups'][number], option: { id: number; name: string; price: number }) => {
  ...
  const handleAddToCart = () => {
    ...
    const resolvedPrice = getDisplayPrice(selectedItem, selectedPlatformId);
    ...
    setCart(prev => {
      ...
      return [...prev, {
        ...
        price:         resolvedPrice,
        deliveryPrices: selectedItem.deliveryPrices ?? [],
        quantity,
  ...
  const clearCart = () => {
    setCart([]);
    clearMemberAndPromo();
    // Reset to first platform on clear
    if (platforms.length > 0) setSelectedPlatformId(platforms[0].id);
  };

  // ── Checkout ──
  const handleCheckout = () => {
    if (cart.length === 0) return toast.error('กรุณาเลือกรายการอาหารก่อน');
    if (!selectedPlatformId) return toast.error('กรุณาระบุแพลตฟอร์ม Delivery ก่อน');
    setIsPaymentDialogOpen(true);
  };

  const handleSubmitOrder = async () => {
    if (!selectedPlatformId) return toast.error('กรุณาระบุแพลตฟอร์ม Delivery');
    ...
    try {
      ...
      toast.success(
        member
          ? `บันทึกออเดอร์สำเร็จ — สะสมแต้มให้ ${member.name} เรียบร้อย`
          : 'บันทึกออเดอร์ Delivery สำเร็จ',
      ...
      if (printerStatus === 'connected') {
        ...
        await printReceipt({
          ...
          tableName:      `Delivery (${selectedPlatform?.name ?? ''})`,
          dateTime:       now,
          paymentType:    'TRANSFER',
          items:          cart.map(item => ({
            name:      item.name,
            qty:       item.quantity,
            unitPrice: item.price + item.options.reduce((s, o) => s + o.price, 0),
  ...
  const filteredItems = (catId: number): MenuItem[] => {
  ...
  // ── Dialog item price (uses resolved price) ──
  const dialogItemPrice = selectedItem ? getDisplayPrice(selectedItem, selectedPlatformId) : 0;
  const dialogTotal = (dialogItemPrice + selectedOptions.reduce((s, o) => s + o.price, 0)) * quantity;

  // ── Shared member-promo props ──
        ...
        <div className="flex flex-col gap-3 sm:gap-4 shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            <h1 className="text-xl sm:text-2xl font-black text-slate-900 flex items-center gap-2 truncate">
              ...
              <span className="truncate">ออเดอร์ Delivery - {branchData?.name}</span>
            </h1>
        ...
        <ScrollArea className="flex-1 rounded-2xl bg-slate-50/50 border border-slate-100">
          <div className="p-2 sm:p-3 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-4">
            {filteredItems(Number(activeTab)).map(item => {
              const displayPrice = getDisplayPrice(item, selectedPlatformId);
              const hasPlatformPrice = selectedPlatformId !== null &&
                item.deliveryPrices?.some(dp => dp.deliveryPlatformId === selectedPlatformId);
              return (
                <Card
                  key={item.id}
                  ...
                  <CardContent className="p-0 flex flex-col h-full">
                    <div className="relative aspect-[4/3] sm:aspect-video overflow-hidden shrink-0">
                      ...
                      <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 z-10 flex flex-col items-end gap-1">
                        ...
                        {hasPlatformPrice && displayPrice !== item.price && (
                          <Badge className="bg-slate-500/80 text-white border-none font-medium shadow-sm text-[9px] line-through">
                  ...
                  <div className="p-4 sm:p-6 space-y-5">
                    <DialogHeader className="space-y-1">
                      <div className="flex justify-between items-start gap-4">
                        <DialogTitle className="text-xl sm:text-2xl font-black leading-tight">
                          {selectedItem.name}
                        </DialogTitle>
                        <div className="flex flex-col items-end shrink-0">
                          <span className="text-lg sm:text-xl font-black text-primary italic">
                            ฿{dialogItemPrice.toLocaleString()}
                          </span>
                          {dialogItemPrice !== selectedItem.price && (
                        ...
                        {selectedItem.optionGroups.map(group => (
                          <div key={group.id} className="space-y-2.5">
                            <div className="flex justify-between items-center px-1">
                              <h4 className="font-black text-slate-400 text-[10px] uppercase tracking-widest leading-none">
                                {group.name}
                              </h4>
                    ...
                    <div className="space-y-2.5 pt-2">
                      <h4 className="font-black text-slate-400 text-[10px] uppercase tracking-widest leading-none px-1">
                        หมายเหตุเพิ่มเติม
                      </h4>
                      <Input
        ...
        <DialogContent className="w-[95vw] sm:max-w-[480px] p-0 overflow-hidden rounded-[24px] sm:rounded-3xl border-none max-h-[95vh] flex flex-col">
            ...
            <div className="p-5 sm:p-7 space-y-6">
              <DialogHeader>
                <DialogTitle className="text-xl sm:text-2xl font-black text-center">
                  ยืนยันออเดอร์ Delivery
                </DialogTitle>
              </DialogHeader>

              <div className="flex items-center justify-between p-4 bg-primary/5 rounded-2xl border border-primary/10">
                <span className="text-sm font-bold text-slate-700">แพลตฟอร์ม:</span>
                <Badge className="bg-primary text-white text-sm font-black px-4 py-1.5 rounded-xl">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">ยอดขายสุทธิ</span>
              ...
              <div className="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100 flex items-start gap-2.5">
                <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                <p className="text-xs text-emerald-700 font-semibold leading-relaxed">
                  ออเดอร์นี้จะถูกบันทึกในสถานะการชำระเงินเป็น{' '}
                  <span className="font-bold">ชำระแล้ว (PAID)</span>{' '}
                  และดึงยอดขายไปรวมในหน้า Dashboard รายงานประจำวันโดยอัตโนมัติ
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-6 bg-white border-t border-slate-100 shrink-0">
            ...
            >
              ...
              <span>ยืนยันและบันทึกออเดอร์ · ฿{finalTotal.toLocaleString()}</span>
            </Button>
          </div>
        </DialogContent>
      </Dialog>
...
function CartSummaryContent({
  ...
  memberPromoProps,
  platforms, selectedPlatformId, setSelectedPlatformId,
}: CartSummaryProps) {
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-3 bg-slate-50/30 no-scrollbar">
        ...
        {/* ── Platform selector (top of cart, always visible) ── */}
        <div className="p-3 border border-slate-100 rounded-2xl bg-white space-y-2">
          ...
          {platforms.length === 0 ? (
            <p className="text-xs text-slate-400 italic">ยังไม่มีแพลตฟอร์ม — ตั้งค่าในหน้าจัดการเมนู</p>
          ) : (
            <Select
              value={selectedPlatformId !== null ? String(selectedPlatformId) : ''}
              onValueChange={(val) => setSelectedPlatformId(Number(val))}
            >
              <SelectTrigger className="w-full h-10 rounded-xl font-bold bg-slate-50 border-slate-100 text-sm">
        ...
        {Array.isArray(cart) && cart.map((item) => (
          <div
            key={item.cartItemId}
            className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col gap-3 group animate-in slide-in-from-right-4 duration-300"
          >
            <div className="flex justify-between items-start gap-4">
              <div className="flex-1 min-w-0">
              ...
              <Button
                variant="ghost" size="icon"
                className="h-8 w-8 text-slate-200 group-hover:text-red-400 shrink-0 transition-colors"
            ...
            <div className="flex justify-between items-center pt-1">
              <div className="flex flex-col">
                <p className="font-black text-primary tabular-nums text-sm">
                  ฿{((item.price + item.options.reduce((s, o) => s + o.price, 0)) * item.quantity).toLocaleString()}
                </p>
                {item.price !== item.basePrice && (
                  <p className="text-[10px] text-slate-400 line-through tabular-nums leading-none">
                    ฿{((item.basePrice + item.options.reduce((s, o) => s + o.price, 0)) * item.quantity).toLocaleString()}
                  </p>
                )}
              </div>
        ...
        {/* ── Member & Promo (only when cart has items) ── */}
        {cart.length > 0 && (
          <div className="pt-1">
      ...
      <div className="p-6 bg-white border-t border-slate-100 space-y-4 shadow-[0_-20px_40px_rgba(0,0,0,0.04)] shrink-0">
        ...
        <Button
          ...
          disabled={isSubmitting || cart.length === 0 || !selectedPlatformId}
          onClick={handleCheckout}
        >
          {isSubmitting ? (
            <><Loader2 className="w-6 h-6 animate-spin" /><span>กำลังบันทึก...</span></>
          ) : (
            <><Calculator className="w-6 h-6" /><span>ตรวจสอบ & ยืนยันออเดอร์</span></>
          )}
        </Button>
      </div>
    </div>
